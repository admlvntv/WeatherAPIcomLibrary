...
public class CurrentWeatherLib {
   ...
   * @throws JsonProcessingException
   */
  public double getCurrentFeelsLikeTemp(String region, char tempScale)
      throws JsonProcessingException {
    switch (tempScale) {
      case 'F':
   ...
   * @param region the region, can be given as US Zipcode, UK Postcode, Canada Postalcode, IP
   *     address, Latitude/Longitude (decimal degree) or city name
   * @param unit the unit of speed, send "mph" or "kph" defaults to kilometers per hour if invalid
   *     unit given
